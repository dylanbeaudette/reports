...
f.subset <- function(i, n) {
	...
	if(nrow(i) > n) {
	  # all columns except the first three: IDs
		idx <- clhs(i[, -c(1:3)], size=n, progress=FALSE, simple=TRUE, iter=1000)
		i.sub <- i[idx, ]
	}
...
f.summary <- function(i, p=c(0.05, 0.25, 0.5, 0.75, 0.95)) {
...
custom.bwplot <- function(x, coef=NA, do.out=FALSE) {
  stats <- quantile(x, p=c(0.05, 0.25, 0.5, 0.75, 0.95))
  n <- length(na.omit(x))
...
if(exists('mu.set')) {
  ...
  mu.set <- mu.set <- unique(mu[[mu.col]])
}
...
## TODO: put this into a function -> sharpshootR
...
for(mu.i in mu.set) {
  ...
  s <- constantDensitySampling(mu.i.sp, n.pts.per.ac=pts.per.acre, min.samples=1, polygon.id='pID')
  
  # make a unique sample ID, need this for conversion: long -> wide
  s$sid <- 1:nrow(s)
  
  ## TODO: estimate effective DF from spatial data
  # http://www.inside-r.org/packages/cran/SpatialPack/docs/modified.ttest
  ...
  a.stats <- round(c(quantile(a, probs=c(0.05, 0.25, 0.5, 0.75, 0.95)), sum(a)))
  names(a.stats) <- c('Q5', 'Q25', 'Median', 'Q75', 'Q95', 'Total Area')
...
# geomorphons
geomorphons.vars <- names(raster.list)[grep('geomorphons', names(raster.list), ignore.case = TRUE)]
if(length(geomorphons.vars) > 0)
  d.geomorphons <- subset(d.mu, subset=variable == geomorphons.vars)

# everything else
d.mu <- subset(d.mu, subset=! variable %in% c(circ.vars, geomorphons.vars))
```
...
### Area Summaries
```{r, echo=FALSE}
kable(mu.area, caption='Map Unit Acreage by Polygon', align = 'r')
```
...
### Tabular Summaries
Table of 5th, 25th, 50th, 75th and 95th [percentiles](https://en.wikipedia.org/wiki/Percentile), by variable.

```{r, echo=FALSE, results='asis'}
# summarize raster data for tabular output
mu.stats <- ddply(d.mu, c('variable', '.id'), f.summary)

# print medians
kable(dcast(mu.stats, .id ~ variable, value.var = 'Q50'), row.names=FALSE, caption = 'Median Values', align = 'r')
```
...
l_ply(split(mu.stats, mu.stats$variable), function(i) {
  ...
  i$variable <- NULL
  print(kable(i, caption = var.name, row.names=FALSE, align = 'r'))
})
...
```{r, echo=FALSE, fig.width=12, fig.height=6, eval=do.geomorphons}
...
## TODO: fix so that auto.key legend is integrated with dendrogramGrob
...
res <- lapply(d.circ.list, function(i) {
  mu <- unique(i$.id)
  circ.stats <- aspect.plot(i$value, q=c(0.05, 0.5, 0.95), plot.title=mu, pch=NA, bg='RoyalBlue', col='black', arrow.col=c('grey', 'red', 'grey'), stack=FALSE, p.bw=90)
...
This plot displays the similarity of the map units across the set of environmental variables used in this report. The contours contain 50% of the points (sub-sampled via cLHS) in the 2D projection of multivariate space.
...
## this can somtimes fail to converge, consider using alternate algorithm
# ordination via sammon mapping
set.seed(10101001)
d.sammon <- sammon(d.dist)

# contour density estimates
s <- data.frame(x=d.sammon$points[, 1], y=d.sammon$points[, 2], .id=d.sub$.id)
s <- split(s, s$.id)

# plot
par(mar=c(1,1,3,1))
plot(d.sammon$points, type='n', axes=FALSE, asp=1)
...
points(d.sammon$points, cex=0.45, col=cols[match(d.sub$.id, mu.set)], pch=16)
...
# compute summaries
poly.stats <- ddply(d.mu, c('pID', 'variable'), f.summary)
