...
<p>
```{r, echo=FALSE, results='asis'}
this.data <- categorical.prop.table(f$ecositeid)
this.align <- rep('c', times=ncol(this.data) + 1)
...
# plot
print(bwplot(genhz ~ clay, data=h.i.sub, main=f, par.settings=tps) + latticeExtra::layer(panel.text(x=h.i.sub$clay, y=jitter(as.numeric(h.i.sub$genhz), factor=1.5), label=h.i.sub$hzname, cex=0.75, font=2, col='RoyalBlue')))
```
...
# group by musym
groupedProfilePlot(f, groups='musym', name='new.hz.name', label='pedon_id', id.style='side', max.depth=max.depth.for.plotting, cex.names=0.8, cex.id=1.2)
```
</p>
...
### Aggregate Color Summary
```{r agg-color-plot, echo=FALSE, fig.width=12, fig.height=8}
par(mar=c(4.5, 2, 0, 0))
aggregateColorPlot(aggregateColor(f, groups = 'genhz', col = 'soil_color'), label.font = 2, label.cex = 0.95, print.n.hz = TRUE)
```
...
```{r diagnostic-prop-plot, echo=FALSE, fig.width=10, fig.height=12}
